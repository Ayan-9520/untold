export default function MusicPromptLibrary({ prompts, category, onUse }) {
  const filtered = prompts?.filter((p) => {
    if (!category) return true;
    const cat = p.parameters?.category;
    return !cat || cat === category;
  }) || [];

  if (!filtered.length) {
    return <p className="text-sm dark:text-untold-muted">No score briefs for this category yet.</p>;
  }
