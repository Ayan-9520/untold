  ...
  return (
    <div className="min-h-dvh dark:bg-untold-dark light:bg-white flex flex-col px-6 pt-[env(safe-area-inset-top)]">
      ...
      <div className="flex-1 flex flex-col justify-center pb-12 max-w-md mx-auto w-full animate-slide-up">
        <h1 className="text-2xl font-bold dark:text-untold-white light:text-black">Join UNTOLD</h1>
        <p className="text-sm dark:text-untold-muted light:text-gray-500 mt-1">The Story Behind The Glory</p>

        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          <input name="name" required placeholder="Full name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputClass} />
          ...
          <input type="password" required minLength={8} placeholder="Password (8+ chars)" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className={inputClass} />
        ...
        <p className="mt-6 text-center text-sm dark:text-untold-muted light:text-gray-500">
          ...
          <Link to="/app/login" className="text-untold-gold font-medium">Sign in</Link>
        </p>
      </div>
    </div>
  );
